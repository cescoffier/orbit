...
public class ScoresCommand implements Runnable {
    ...
    public ScoresCommand(AnalysisService analysisService) {
    ...
    public void run() {
        Optional<PrPulseConfig.Repository> repoConfig = analysisService.findRepoConfig(repo);
        if (repoConfig.isEmpty()) {
            String known = analysisService.knownRepos().stream()
                    .map(r -> "  - " + r)
        ...
        if (refresh) {
            ...
            try {
                List<ScoredPullRequest> scored = analysisService.analyzeSingleRepo(repo);
    ...
    void displayScores(String owner, String name, String fullName) {
        ...
        List<PullRequestScore> scores = PullRequestScore.findByRepo(repoEntity.get(), limit);
        if (scores.isEmpty()) {
            System.out.println("No scores stored for " + fullName + ". Use --refresh to fetch and score.");
            return;
        }

        // Eagerly fetch details to avoid lazy loading issues
        for (PullRequestScore score : scores) {
            score.details.size(); // force initialization
        }
        ...
        for (int i = 0; i < scores.size(); i++) {
            PullRequestScore score = scores.get(i);
            ...
            if (details) {
                for (ScoreDetail detail : score.details) {
